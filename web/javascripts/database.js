$(function () {
  ...
  init_title(links.GI[4].Name[lang2] + ' ' + computer_.MiscText.PAGE_TITLE[lang]);
  $('.tls' + lang).css("color", "#df903b");
  $('h3 .title').html(computer_.MiscText.AbyssTitle[lang2] + "<color style='font-size: 26px;'><br>" + computer_.MiscText.game_img[lang] + VER_GI + "</b></color>");
  ...
  function begin1() {
    ...
    script_computer_2.onload = function () {
      _SpiralAbyssFloorConfig = {
        ..._SpiralAbyssFloorConfig,
        ..._SpiralAbyssFloorConfig_2
      };
      did_2_load = 1;
    };
        ...
        section: function (d) {
          ...
          for (var i = 0; i < SpiralAbyssDPSDatas.length; i++) {
            ...
            for (var j = 0; j < SpiralAbyssDPSData.Data.length; j++) {
              if (value_sum.length <= j) value_sum.push(0);
              label_array.push(SpiralAbyssDPSData.Data[j].Ver);
              var dps_ = dps(SpiralAbyssDPSData.Chamber, SpiralAbyssDPSData.Data[j].DPS, SpiralAbyssDPSData.Data[j].Ver);
              value_array.push(dps_);
              value_sum[j] += dps_;
            }
  ...
  $('body').on('click', '.dl_button', function () {
    ...
    $('.temp').remove();
    $('.content').render({
      a: "/Abyss/".concat(use_lang, "/").concat(cur_schedule_ver, ".png"),
      attr: {
        download: "".concat(cur_schedule_ver, " ").concat(spiral_abyss_name, ".png")
      },
      style: {
        display: 'none'
      },
  ...
  function renderFloorPre(index) {
    var _this_generation = parseInt($(this).attr('data-gen'));
    if (did_2_load || _this_generation <= 3 && _this_generation >= 99) {
      renderFloor_Actual(index);
    } else {
      //$('.lt').show()
      var fffgu = setInterval(function () {
        if (did_2_load) {
          //$('.lt').hide()
          clearInterval(fffgu);
          renderFloor_Actual(index);
  ...
  function renderFloor_Actual(index) {
    ...
    p_b.forEach(function (w) {
      p_b_show.push("<b>".concat(w.b.Name, "</b>"));
                ...
                template: {
                  li: [{
                    h6: [{
                      ...
                      span: cur_schedule_id == 99 && !cur_real ? computer_.MiscText.Abyss_Reminder2[lang] : computer_.MiscText.Abyss_Reminder[lang],
                      class: 'avd',
                      style: {
                        'display': 'block',
                        'color': '#6f6f6f',
                        'font-weight': '500',
                        'margin-bottom': cur_schedule_id == 99 ? '14px' : ''
                      }
                    }, {
                      div: {
                        div: '',
                        ...
                        div: {
                          div: function (p) {
                            ...
                            if (!weav) {
                              ...
                              if (_mon_.Name) {
                                _name = "<color style=\"color:".concat(_color, ";\"><b>") + _mon_.Name + '</b></color>';
                              } else if (_Monsters[_mon_.ID] && _Monsters[_mon_.ID].UseCustomColorName) {
                                _name = '<b>' + _MonsterCustomColorNameConfig[_mon_.ID].Name + '</b>';
                              } else {
                                _name = "<color style=\"color:".concat(_color, ";\"><b>") + (_Monsters[_mon_.ID] ? _Monsters[_mon_.ID].Name : '???') + '</b></color>';
                              }
                              if (p.data.Monsters.length == 1) {
                                ...
                                if (lang == 'CH' && _Monsters[_mon_.ID] && _Monsters[_mon_.ID].Title) {
                                  title_show = 1;
                                  this_title = _Monsters[_mon_.ID].Title;
                                }
                              } else {
                                weav = {
                                  Show: {
    ...
    var widtrh = "calc((100% / ".concat(row_count, ") - 8px)");
    for (const g of guide) {
      $('.guides').render({
        ...
        class: "a_section guide_".concat(g),
        id: "guide_".concat(g),
        style: {
          width: widtrh,
          'max-width': '500px'
  ...
  $('body').on('click', '.guide_jump', function (s) {
  ...
  function Floor_Render(interval, Mons, MonCount_0) {
    ...
    Mons.forEach(function (mid) {
      ...
      var mon_hp = num.toFixed(0) + (monster.HPCount ? "\xD7".concat(monster.HPCount) : '');
      var has_guide = guide.includes(mid);
      var has_skill = monster.Csx && monster.Csx.length && lang == 'CH';
      var skill_str = '';
      if (has_skill) skill_str = monster.Csx[0];
      monsters_stuff.push({
        div: [{
          ...
          t: [{
            ...
            /*{
                p: computer_.MiscText.LocalLegend[lang],
                class: 'monster_hp',
                style: {
                    color: '#666'
                },
          ...
          when: has_guide || has_skill
        }],
        class: 'monster_card_2'
      });
    });
    var monster_final_name = _MonsterCustomColorNameConfig[Mons[0]] ? _MonsterCustomColorNameConfig[Mons[0]].Name : _Monsters[Mons[0]].Name;
    var monster_title = _Monsters[Mons[0]].Title;
    return [{
      p: computer_.MiscText.AbyssCount[lang].replace("#", MonCount),
      class: 'enemy_count',
      when: MonCount != 1 && MonCount != 0
    }, {
      p: monster_final_name,
      class: 'enemy_count',
      ...
      p: monster_title,
      class: 'monster_title_2',
      when: (MonCount == 1 || Mons.length == 1) && lang == 'CH'
    }, {
      div: monsters_stuff,
  ...
  function trigger_hp_coeff(interval) {
    if (hp_coeff_single_cache) {
      $('.hp_coeff').html("HP <color style=\"color:#cc0000\">".concat((hp_coeff_single_cache * 100).toFixed(0), "%</color>"));
      hp_coeff_cache_show_and_link = hp_coeff_single_cache;
    } else {
      $('.hp_coeff').html("HP <color style=\"color:#cc0000\">".concat((_SpiralAbyssFloorEntryToLevelCoeffConfig[interval].HPCoeff * 100).toFixed(0), "%</color>"));
      ...
      moster: {
        div: [{
          div: function (m) {
            ...
            var has_guide = guide.includes(m.data.ID);
            var has_skill = monster.Csx && monster.Csx.length && lang == 'CH';
            var skill_str = '';
            if (has_skill) skill_str = monster.Csx[0];
            $(m.container).render([{
              ...
              div: [{
                span: function (d) {
                  ...
                  if (HP == 0) HP = '';
                  var s = '<b><color style="color:' + (computer_.TextColorConfig[monster.Color] || '') + ';">' + HP + '</color>' + (monster.HPCount ? "\xD7".concat(monster.HPCount) : '') + '</b>';
  ...
  $('body').on('click', '.monicon_container', function () {
