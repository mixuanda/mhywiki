$(function () {
  ...
  function writeData() {
    ...
    if (buff.Extra && buff.Extra.length) {
      buff.Extra.forEach(function (t) {
        ...
        if (t.DMG) {
          ...
          if (t.Color) {
            affix = "<color style='color:#".concat(elemcolor[t.Color], "'>") + affix + '</color>';
          }
        }
        $('.buff_desc').render({
          p: '- ' + t.Name + ' ' + affix,
    ...
    cur_phase_data.UC.forEach(function (w) {
      v = _mb[w];
      show = "<b>".concat(w, " ").concat(v.Name, "</b><br>").concat(v.Desc);
      for (ee_id of v.EE) {
        if (!uc_ee_list.includes(ee_id)) {
          show += "<br><b>".concat(_ee[ee_id].Name, "</b><br>").concat(_ee[ee_id].Desc);
        }
    ...
    cur_phase_data.UB.forEach(function (v) {
      $('.u_b_b').render({
        p: "<b>".concat(v._id, " ").concat(v.Name, "</b><br>").concat(v.Desc),
        style: {
          'line-height': '1.7',
          'margin-bottom': '0'
        }
  ...
  function RES(L) {
    ...
    if (_monster[max_id] && _monster[max_id]["9"]) {
      ...
      for (const [p, q] of Object.entries(_monster[max_id]["9"])) {
        reses.push({
          div: [{
            img: imgpre + "images/Element/".concat(p, ".png"),
        ...
        span: [{
          ...
          div: [{
            span: function () {
              ...
              return s + (me["5"] ? " <color style='color:#666;'>[".concat(((stage_eg ? stage_eg.HP ? stage_eg.HP : 1 : 1) * me["5"] * 100).toFixed(0), "%]</color>") : '');
            },
            class: 'monname_2',
            when: t.HP
          }, {
            ...
            span: '<b><color style="color:#2545ba;">' + t.SPD.toString() + '</color></b>' + (me["6"] ? " <color style='color:#666;'>[".concat(((stage_eg ? stage_eg.SPD ? stage_eg.SPD : 1 : 1) * me["6"] * 100).toFixed(0), "%]</color>") : ''),
            class: 'monname_2',
            when: t.SPD
          }, {
            br: '',
