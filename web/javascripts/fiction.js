$(function () {
        ...
        span: [{
          ...
          div: [{
            span: function () {
              ...
              return s + "<span class=\"csx\"> <color style='color:#666;'>[".concat(((stage_eg ? stage_eg.HP ? stage_eg.HP : 1 : 1) * (me["5"] ? me["5"] : 1) * 100 * (1 + (hp_add ? hp_add : 0))).toFixed(0), "%]</color></span>");
            },
            class: 'monname',
            when: t.HP
          }, {
  ...
  $('body').on('click', '.monster_card', function () {
    window.open("monster?lang=".concat(lang3, "&id=").concat($(this).attr('data-id'), "&lv=").concat($(this).attr('data-lv'), "&hlg=").concat($(this).attr('data-hl'), "&eg=").concat($(this).attr('data-eg'), "&def=1000"));
  });
  ...
  $('body').on('click', '.dl_button', function () {
    ...
    $('.temp p').click();
    //gtag('event', 'download_sr_fiction', {
    //    'type': 'sr_fiction'
    //})
