$(function () {
  ...
  function begin() {
    ...
    _title = lang == 'CH' ? '星穹铁道 文本+剧情搜索' : 'HSR Text+Dialogue Search';
  ...
  $('body').on('click', '.choice_4', function () {
    window.open('/sr/readable', '_blank');
  });
  function activate_1() {
    ...
    if (loading == 1) {
      loading = 2;
      fetch('/TextMap/SR.json').then(x => x.json()).then(y => {
        _DATA = y;
        loading = 0;
  ...
  function activate_2() {
    ...
    if (loading_2 == 1) {
      loading_2 = 2;
      fetch("/TextMap/SR_Talk_".concat(lang, ".json")).then(x => x.json()).then(y => {
        _DATA_2 = y;
        loading_2 = 0;
  ...
  function text(t) {
    ...
    while (temp.includes('<color=')) {
      ...
      if (color == "#FFFFFFFF") temp = temp.replaceAll(temp.substring(start, end + 1), "<color style='color:".concat(color, ";background-color:#27363E'>"));else temp = temp.replaceAll(temp.substring(start, end + 1), "<color style='color:".concat(color, "'>"));
    }
    return temp;
  }
