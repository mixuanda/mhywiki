$(function () {
  ...
  function begin() {
    ...
    _map.forEach(function (o, j) {
      $('.rar').render({
        schedule: o.Name,
        class: j + 1 == place ? 'active' : '',
        a: {
          'data-id': j + 1
        }
      });
  ...
  function renderMap(k) {
    ...
    var xlist = [];
    var ylist = [];
    for (const [n, p] of Object.entries(_map[place - 1].Parts)) {
      var spl = n.split("_");
    ...
    for (const i in dat) {
      $('#map').render({
        ...
        style: {
          position: 'absolute',
          left: (dat[i].X - xmin) * unit + 'px',
          top: (dat[i].Y - ymin) * unit + 'px',
          width: "".concat(unit, "px"),
          height: "".concat(unit, "px"),
          'background-image': "url(".concat(imgpre, "images/Map/").concat(dat[i].Name, ".png)"),
  ...
  $('body').on('click', '.plus', function () {
    ...
    scale += 0.05;
    $('.mid').html(parseInt(scale * 100) + '%');
    $('#map').css('transform', "scale(".concat(scale, ")"));
    $('.area').css('height', ylen * unit * scale + 'px');
  });
