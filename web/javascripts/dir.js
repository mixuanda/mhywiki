var _quest_allow = 1

var GI = [
    {
        "Title": {
            "CH": "怪物",
            "EN": "Monsters",
            "RU": "Противники"
        },
        "Link": {
            "CH": "mons.wiki",
            "EN": "en.mons.wiki",
            "RU": "en.mons.wiki"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/monster",
            "EN": "/gi/monster",
            "RU": "/gi/monster"
        }
    },
    {
        "Title": {
            "CH": "角色 武器 圣遗物",
            "EN": "Characters Weapons Artifacts",
            "RU": "Персонажи Оружие Артефакты"
        },
        "Link": {
            "CH": "char.wiki",
            "EN": "en.char.wiki",
            "RU": "en.char.wiki"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/char",
            "EN": "/gi/char",
            "RU": "/gi/char"
        }
    },
    {
        "Title": {
            "CH": "刻晴成绩",
            "EN": "Keqing Endgame",
        },
        "Features": [],
        "Href": {
            "CH": "/gi/keqing",
            "EN": "/gi/keqing",
            "RU": "/gi/keqing"
        }
    },
    {
        "Title": {
            "CH": "幽境危战",
            "EN": "Stygian Onslaught"
        },
        "Link": {
            "CH": "abyss.wiki",
            "EN": "en.abyss.wiki",
            "RU": "en.abyss.wiki"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/3boss",
            "EN": "/gi/3boss",
            "RU": "/gi/3boss"
        }
    },
    {
        "Title": {
            "CH": "深境螺旋",
            "EN": "Spiral Abyss",
            "RU": "Витая Бездна"
        },
        "Link": {
            "CH": "abyss.wiki",
            "EN": "en.abyss.wiki",
            "RU": "en.abyss.wiki"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/abyss",
            "EN": "/gi/abyss",
            "RU": "/gi/abyss"
        }
    },
    {
        "Title": {
            "CH": "幻想真境剧诗",
            "EN": "Imaginarium Theater",
            "RU": "Театр Воображариум"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/maze",
            "EN": "/gi/maze",
            "RU": "/gi/maze"
        }
    },
    {
        "Title": {
            "CH": "祈愿卡池",
            "EN": "Gacha Banners",
            "RU": "Gacha Banners"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/banner",
            "EN": "/gi/banner",
            "RU": "/gi/banner"
        }
    },
    {
        "Title": {
            "CH": "剧情记录<color style='color:#FFD780'>（新新）</color>",
            "EN": "Quest Logs <color style='color:#FFD780'>[NewNew]</color>",
            "RU": "Задания, Истории"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/quests",
            "EN": "/gi/quests",
            "RU": "/gi/quests"
        },
        when: _quest_allow,
    },
    {
        "Title": {
            "CH": "生日日历<color style='color:#FFD780'>（新）</color>",
            "EN": "Birthday Calendar <color style='color:#FFD780'>[New]</color>",
            "RU": "Задания, Истории"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/birthday",
            "EN": "/gi/birthday",
            "RU": "/gi/birthday"
        },
    },
    {
        "Title": {
            "CH": "材料 食物 名片 头像",
            "EN": "Items Food Namecards Avatars",
            "RU": "Материал, Еда, Именные карта"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/item",
            "EN": "/gi/item",
            "RU": "/gi/item"
        }
    },
    {
        "Title": {
            "CH": "成就",
            "EN": "Achievements",
            "RU": "Достижения"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/ach",
            "EN": "/gi/ach",
            "RU": "/gi/ach"
        }
    },
    {
        "Title": {
            "CH": "地理志",
            "EN": "Geography Codex",
            "RU": "География"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/geo",
            "EN": "/gi/geo",
            "RU": "/gi/geo"
        }
    },
    {
        "Title": {
            "CH": "教程",
            "EN": "Tutorials",
            "RU": "Обучение"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/tut",
            "EN": "/gi/tut",
            "RU": "/gi/tut"
        }
    },
    {
        "Title": {
            "CH": "活动",
            "EN": "Events",
            "RU": "Событие"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/event",
            "EN": "/gi/event",
            "RU": "/gi/event"
        }
    },
    {
        "Title": {
            "CH": "加载提示",
            "EN": "Loading Tips",
            "RU": "Подсказки на экране загрузки"
        },
        "Features": [],
        "Href": {
            "CH": "/gi/load",
            "EN": "/gi/load",
            "RU": "/gi/load"
        }
    },
    {
        "Title": {
            "CH": "妮可少女的DIY小屋",
            "EN": "",
            "RU": ""
        },
        "Features": [],
        "Href": {
            "CH": "/gi/diy",
            "EN": "",
            "RU": ""
        }
    },
]

var SR = [
    {
        "Title": {
            "CH": "怪物",
            "EN": "Monsters"
        },
        "Link": {
            "CH": "sr.mons.wiki",
            "EN": "sr.mons.wiki"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/monster",
            "EN": "/sr/monster"
        }
    },
    {
        "Title": {
            "CH": "角色 光锥 遗器",
            "EN": "Characters Lightcones Relics"
        },
        "Link": {
            "CH": "sr.char.wiki",
            "EN": "sr.char.wiki"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/char",
            "EN": "/sr/char"
        }
    },
    {
        "Title": {
            "CH": "云璃成绩",
            "EN": "Yunli Endgame"
        },
        "Features": [],
        "Href": {
            "CH": "/yunli",
            "EN": "/yunli"
        }
    },
    {
        "Title": {
            "CH": "混沌回忆",
            "EN": "Memory of Chaos"
        },
        "Link": {
            "CH": "sr.abyss.wiki",
            "EN": "sr.abyss.wiki"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/chaos",
            "EN": "/sr/chaos"
        }
    },
    {
        "Title": {
            "CH": "虚构叙事",
            "EN": "Pure Fiction"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/fiction",
            "EN": "/sr/fiction"
        }
    },
    {
        "Title": {
            "CH": "末日幻影",
            "EN": "Apocalyptic Shadow"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/shadow",
            "EN": "/sr/shadow"
        }
    },
    {
        "Title": {
            "CH": "异相仲裁",
            "EN": "Anomaly Arbitration"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/arbitration/",
            "EN": "/sr/arbitration/"
        }
    },
    {
        "Title": {
            "CH": "跃迁卡池",
            "EN": "Gacha Banners"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/banner",
            "EN": "/sr/banner"
        }
    },
    {
        "Title": {
            "CH": "差分宇宙 3.8",
            "EN": "Divergent Universe 3.8"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/du31",
            "EN": "/sr/du31"
        }
    },
    {
        "Title": {
            "CH": "差分宇宙 2.7",
            "EN": "Divergent Universe 2.7"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/du",
            "EN": "/sr/du"
        }
    },
    {
        "Title": {
            "CH": "台词数统计",
            "EN": "Line Count"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/linecount",
            "EN": "/sr/linecount"
        }
    },
    {
        "Title": {
            "CH": "物品",
            "EN": "Items"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/item",
            "EN": "/sr/item"
        }
    },
    {
        "Title": {
            "CH": "教程",
            "EN": "Tutorials",
        },
        "Features": [],
        "Href": {
            "CH": "/sr/tut",
            "EN": "/sr/tut",
        }
    },
    {
        "Title": {
            "CH": "成就",
            "EN": "Achievements"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/ach",
            "EN": "/sr/ach"
        }
    },
    {
        "Title": {
            "CH": "活动",
            "EN": "Events"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/event",
            "EN": "/sr/event"
        }
    },
    {
        "Title": {
            "CH": "战斗关卡",
            "EN": "Combat Levels"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/stage",
            "EN": "/sr/stage"
        }
    },
    {
        "Title": {
            "CH": "效果和名词",
            "EN": "Effects & Nouns"
        },
        "Features": [],
        "Href": {
            "CH": "/sr/effect",
            "EN": "/sr/effect"
        }
    },
]