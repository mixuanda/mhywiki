$(function () {
            ...
            div: [{
              span: ["HP ", {
                i: function (d) {
                  ...
                  return num.toFixed(0) + (cur_hp_count ? "\xD7".concat(cur_hp_count) : '');
                }
              }]
            }, {
              span: ["ATK ", {
                  ...
                  template: {
                    tr: [{
                      ...
                      if (p.data.RESModify && p.data.RESModify.length) {
                        if ($(d.container).parents('tbody').attr("".concat(d.data.State, "-modify")) == 1) {
                          return;
                        }
                        $(d.container).render({
          ...
          if (is_tutorial) {
            ...
            $(c.container).render({
              div: [{
                  ...
                  if (data.Tut) {
                    _Tuts[data.Tut].forEach(function (e) {
                      e.I.forEach(function (f) {
                        $(c.container).render({
                          img: imgpre + "images/Tutorial/".concat(f, ".png"),
                          style: {
                            'margin-top': '25px',
                            'margin-bottom': '-10px'
                          }
                ...
                p: '<b>' + cur_interval + ' ' + (mul * 100).toFixed(0) + '% <color style="color:#FFD780;">' + cur_hp.toFixed(0) + (cur_hp_count ? "\xD7".concat(cur_hp_count) : '') + '</color></b>',
                style: {
                  'font-size': '1.05em',
                  'text-align': 'center',
                  'margin': '0'
      ...
      l.data.Monsters.forEach(function (mn) {
        ...
        $(l.container).render({
          ...
          a: {
            "data-id": "".concat(mn),
            "data-fc": '',
            class: "hover-shadow monster_".concat(mn)
          },
          event: {
            click: function (d) {
  ...
  function hasInput() {
    ...
    if (m_object) {
      ...
      var ele = $('.kingdom').find(".kingdom_".concat(kdm));
      ele.addClass('active');
      cur_kingdom = _Kingdoms[ele.index()];
      monsterRenderPre(_Kingdoms[ele.index()]);
      $('.class_monster').find(".monster_".concat(MONSTERID)).addClass('active');
      $('.calculate').click();
    }
  }
  ...
  function process(txt) {
    ...
    for (const [i, j] of Object.entries(computer_.TextColorLightConfig)) {
      t = t.replaceAll("<".concat(i, ">"), "<color style='color:".concat(j, "'>"));
    }
    return t;
  }
