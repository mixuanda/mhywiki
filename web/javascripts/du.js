$(function () {
  ...
  init_title(name_[lang3] + (avid == '2' ? ' 3.8' : '2.7') + ' ' + txt.PAGE_TITLE[lang]);
  function begin() {
    ml = 0;
    cur_star = 4;
    ...
    if (avid == '2') Select_Titan();else Select_2();
  }
  function Select_1() {
    var now = Date.now();
    ...
    this_data.Blessings.forEach(function (u) {
      ...
      $('#B').render({
        template: {
          ...
          style: t.Effects && t.Effects.length ? {
            cursor: 'pointer'
          } : {}
        }
      });
