$(function () {
  ...
  function writeData() {
    ...
    for (const z of [0, 1, 2, 3, 4]) {
      var key = ['TrialA', 'TrialB', 'TrialC', 'FinalEasy', 'FinalHard'][z];
      var buff_key = ['BuffA', 'BuffB', 'BuffC', 'FinalBuffs', 'FinalBuffs'][z];
      var tag_key = ['', '', '', 'FinalTagsEasy', 'FinalTagsHard'][z];
      var elem_key = ['ElemA', 'ElemB', 'ElemC', 'ElemFinal', 'ElemFinal'][z];
      var letter = ['a', 'b', 'c', 'd', 'e'][z];
      var show_text = [lang == 'CH' ? '骑士一' : 'Knight I', lang == 'CH' ? '骑士二' : 'Knight II', lang == 'CH' ? '骑士三' : 'Knight III', lang == 'CH' ? '将杀王棋' : 'Checkmate', lang == 'CH' ? '将杀王棋•绝境' : 'Checkmate: Zugzwang'][z];
      $(".".concat(letter, "_r")).empty().render({
        template: [{
          div: [{
      ...
      if (z < 3) {
        ...
        for (const buff_data of cur_floor_data[buff_key]) {
          need_desc.push({
            p: "<b>".concat(buff_data._id, " ").concat(buff_data.Name, "</b><br>").concat(buff_data.Desc),
            class: 'smallbuff_desc'
          });
        }
        $(".a_b_".concat(z)).empty().render(need_desc);
      } else {
        $(".".concat(letter, "_b")).empty();
        for (const b of cur_floor_data[buff_key]) {
          $(".".concat(letter, "_b")).render({
            template: {
              div: [{
                p: b.Name + '<br>' + b._id,
                class: 'smallbuff_name'
        ...
        $(".d_e_".concat(z)).empty().render(need_desc);
      }
    }
    $('.u_g').empty();
  ...
  function generate_boss_guide(p, d) {
    ...
    d.forEach(function (i) {
      ...
      $(p).render({
        ...
        class: "a_section a_section_split_".concat(split_index)
      });
    });
  }
  ...
  $('body').on('click', '.monster_card', function () {
    window.open("/sr/monster?lang=".concat(lang3, "&id=").concat($(this).attr('data-id'), "&lv=").concat($(this).attr('data-lv'), "&hlg=").concat($(this).attr('data-hl'), "&eg=").concat($(this).attr('data-eg'), "&def=1000"));
  });
  ...
  $('body').on('click', '.dl_button', function () {
    ...
    $('.content').render({
      ...
      attr: {
        download: "".concat(cur_schedule_ver, ".png")
      },
      style: {
        display: 'none'
      },
    ...
    $('.temp p').click();
    //gtag('event', 'download_sr_arbitration', {
    //    'type': 'sr_arbitration'
    //})
