$(function () {
    ...
    $('body').on('click', '.guide_jump', function (s) {
        ...
        if (sk) {
            window.open(`https://homdgcat.wiki/EnemyChart/${sk}.png`, '_blank')
        }
        document.getElementById(`guide_${id}`).scrollIntoView({ behavior: "smooth", block: "center" })
    })
    ...
    function Floor_Render(interval, Mons, MonCount_0) {
        ...
        Mons.forEach(function (mid) {
            ...
            var mon_hp = num.toFixed(0) + (monster.HPCount ? `×${monster.HPCount}` : '')
                                    ...
                                    div: [
                                        {
                                            span: function (d) {
                                                ...
                                                if (HP == 0) HP = ''
                                                var s = '<b><color style="color:' + (computer_.TextColorConfig[monster.Color] || '') + ';">' + HP + '</color>'  + (monster.HPCount ? `×${monster.HPCount}` : '') + '</b>'
